...
private[spark] class Client(
  ...
  def setRMRequestState(): Unit = {
    SparkContext.setMETrigger(true)
    logInfo(s"")
    logInfo(s"RMReuest is true")
    logInfo(s"")
  }
  ...
  def monitorApplication(
                          ...
                          logApplicationReport: Boolean = true): (YarnApplicationState, FinalApplicationStatus, RMRequestState) = {
    val interval = sparkConf.getLong("spark.yarn.report.interval", 1000)
    var lastState: YarnApplicationState = null
    var rmRequestState: RMRequestState = null
    while (true) {
      ...
      val report: ApplicationReport =
        try {
          ...
          case e: ApplicationNotFoundException =>
            ...
            return (YarnApplicationState.KILLED, FinalApplicationStatus.KILLED, RMRequestState.NoRMRequest)
          case NonFatal(e) =>
            logError(s"Failed to contact YARN for application $appId.", e)
            return (YarnApplicationState.FAILED, FinalApplicationStatus.FAILED, RMRequestState.NoRMRequest)
        }
      ...
      if (state == YarnApplicationState.FINISHED ||
        ...
        cleanupStagingDir(appId)
        return (state, report.getFinalApplicationStatus, report.getRMRequestState)
      }

      if (state == YarnApplicationState.RUNNING && rmRequestState == RMRequestState.RMRequest){
        setRMRequestState()
      }
      ...
      lastState = state
      rmRequestState = report.getRMRequestState
    }
  ...
  def run(): Unit = {
    ...
    if (fireAndForget) {
      ...
      val state = report.getYarnApplicationState
      var rmRequestState = report.getRMRequestState
      logInfo(s"Application report for $appId (state: $state)")
      logInfo(formatReportDetails(report))
      if (state == YarnApplicationState.RUNNING && rmRequestState == RMRequestState.RMRequest) { setRMRequestState()}
      if (state == YarnApplicationState.FAILED || state == YarnApplicationState.KILLED) {
        throw new SparkException(s"Application $appId finished with status: $state")
      }
    } else {
      val (yarnApplicationState, finalApplicationStatus, rmRequestState) = monitorApplication(appId)
