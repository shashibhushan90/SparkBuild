...
import org.apache.hadoop.yarn.api.records.{ApplicationId, YarnApplicationState, RMRequestState}
...
private[spark] class YarnClientSchedulerBackend(
  ...
  private def asyncMonitorApplication(): Thread = {
    ...
    val t = new Thread {
      override def run() {
        try {
          val (state, _, _) = client.monitorApplication(appId, returnOnRunning = false, logApplicationReport = false)
