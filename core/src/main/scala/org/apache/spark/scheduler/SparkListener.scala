...
//test
@DeveloperApi
case class SparkListenerRMRequest(evalTrigger: Boolean = true)  extends SparkListenerEvent
...
trait SparkListener {
  ...
  /**
   * Called when RM Requests for resources
   */
  def onRMRequest(rmRequest: SparkListenerRMRequest): Boolean = { true}
