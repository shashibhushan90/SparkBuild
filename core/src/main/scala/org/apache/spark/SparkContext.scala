 ...
 */
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient with METrigger {
  ...
  /** RMRequest parameters
  //val applistener = new ApplicationEventListener
  //var Evaltrigger: Boolean = false
  ...
  /** Trying the RMRequest here
  def checkRMRequest(): Boolean = {

    Evaltrigger = applistener.onRMRequest(SparkListenerRMRequest.apply(true))
    if (Evaltrigger == true) {
      return true
    }
    else {
 ...
 */
object SparkContext extends Logging with METrigger{
 ...
 override def setMETrigger(state: Boolean): Unit ={
   RMRequest = state

  }

  override  def getMETrigger(): Boolean ={
    return RMRequest
  }
