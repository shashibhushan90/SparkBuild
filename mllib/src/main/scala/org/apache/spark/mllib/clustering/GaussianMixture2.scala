...
class GaussianMixture private (
  ...
  def run(data: RDD[Vector], mypath: String = "/user/hduser/"): GaussianMixtureModel = {
    ...
    while (iter < maxIterations && math.abs(llh-llhp) > convergenceTol) {
      ...
      if (is10percent(iter) == true) {

        val toggle: Boolean = true
        logInfo(s"----------------------------------------------------------------------------------------------")
        logInfo(s"----------------------------------------------------------------------------------------------")
        logInfo(s"------------------------------Check pointing at iteration: $iter ---------------------------------")
        logInfo(s"----------------------------------------------------------------------------------------------")
        logInfo(s"----------------------------------------------------------------------------------------------")
        logInfo(s"----------------------------------------------------------------------------------------------")
        //trait compare
        val tempModel = new GaussianMixtureModel(weights, gaussians)
        val savePath: String = mypath + sc.applicationId + "Iteration" + iter
        logInfo(s"Model at iteration number: $iter is saved in $savePath")
      ...
      /*if( rmRequest == true) {
        ...
        val arraySize = 3
        val WSErate = 0.95
        if (sqErrors.size >= arraySize) {
          val WSEArray = sqErrors.toArray

          if ((WSEArray {1}.toString.toDouble / WSEArray{2}.toString.toDouble) > WSErate && (WSEArray{2}.toString.toDouble / WSEArray{3}.toString.toDouble > WSErate)) {
            loop.break()
          }
